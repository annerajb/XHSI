...
//	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_INDICATORS_AIRSPEED_KTS_PILOT );
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(airspeed_pilot));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_INDICATORS_AIRSPEED_KTS_COPILOT );
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(airspeed_copilot));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_INDICATORS_ALTITUDE_FT_PILOT );
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(altitude_pilot));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_INDICATORS_ALTITUDE_FT_COPILOT );
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(altitude_copilot));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_INDICATORS_VVI_FPM_PILOT );
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(vvi_pilot));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_INDICATORS_VVI_FPM_COPILOT );
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(vvi_copilot));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_INDICATORS_SIDESLIP_DEGREES );
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(sideslip));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_ACTUATORS_RADIO_ALTIMETER_BUG_FT_PILOT );
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(ra_bug_pilot));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_ACTUATORS_RADIO_ALTIMETER_BUG_FT_COPILOT );
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(ra_bug_copilot));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_ACTUATORS_BAROMETER_SETTING_IN_HG_PILOT );
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(baro_pilot));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_ACTUATORS_BAROMETER_SETTING_IN_HG_COPILOT );
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(baro_copilot));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_GAUGES_INDICATORS_AIRSPEED_ACCELERATION );
	...
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_WEATHER_TEMPERATURE_AMBIENT_C);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(oat));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_WEATHER_SPEED_SOUND_MS);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(sound_speed));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_TIME_TIMER_IS_RUNNING_SEC);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(timer_is_running));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_TIME_TIMER_ELAPSED_TIME_SEC);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(elapsed_time_sec));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_TIME_TOTAL_FLIGHT_TIME_SEC);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(flight_time_sec));
	i++;


	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_MASTER_CAUTION );
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(master_caution));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_MASTER_WARNING );
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(master_warning));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_CONTROLS_GEAR_HANDLE_DOWN );
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(gear_handle));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_GEAR_UNSAFE );
	...
	sim_packet.sim_data_points[i].id = custom_htonl((long) XHSI_AIRCRAFT_GEAR_COUNT);
	sim_packet.sim_data_points[i].value = custom_htonf((float) gears);
	i++;

	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_CONTROLS_PARKING_BRAKE_RATIO);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(parkbrake_ratio));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_CONTROLS_FLAP_RATIO);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(flap_deploy));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_CONTROLS_FLAP_HANDLE_DEPLOY_RATIO);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(flap_handle));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_CONTROLS_ACF_FLAP_DETENTS);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(flap_detents));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_AUTOPILOT_DISCONNECT);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(ap_disc));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_FUEL_QUANTITY);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(low_fuel));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_GPWS);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(gpws));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_ICE);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(ice));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_PITOT_HEAT);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(pitot_heat));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_STALL_WARNING);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(stall));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_GEAR_WARNING);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(gear_warning));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_SWITCHES_AUTO_BRAKE_LEVEL);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(auto_brake_level));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_CONTROLS_SPEEDBRAKE_RATIO);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(speedbrake_handle));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_FLIGHTMODEL2_CONTROLS_SPEEDBRAKE_RATIO);
	...
	for (g=0; g<gears; g++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL2_GEAR_DEPLOY_RATIO_ + g));
        sim_packet.sim_data_points[i].value = custom_htonf( gear_ratio[g] );
        i++;
	}


    sim_packet.sim_data_points[i].id = custom_htonl((long) CL30_STATUS);
    sim_packet.sim_data_points[i].value = custom_htonf((float) cl30_ready);
    i++;
    if ( cl30_ready ) {

        sim_packet.sim_data_points[i].id = custom_htonl((long) CL30_MAST_WARN);
        sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(cl30_mast_warn));
        i++;
        sim_packet.sim_data_points[i].id = custom_htonl((long) CL30_MAST_CAUT);
...
//	float gear_ratio[10];
	...
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT_ELECTRICAL_BATTERY_ON);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(battery_on));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT_ELECTRICAL_COCKPIT_LIGHTS_ON);
	...
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_RADIOS_INDICATORS_OUTER_MARKER_LIT );
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(outer_marker));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_RADIOS_INDICATORS_MIDDLE_MARKER_LIT );
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(middle_marker));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_RADIOS_INDICATORS_INNER_MARKER_LIT );
	...
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) XHSI_EFIS_PILOT_DA_BUG);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(efis_pilot_da_bug));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) XHSI_EFIS_PILOT_MINS_MODE);
	...
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) XHSI_EFIS_COPILOT_DA_BUG);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(efis_copilot_da_bug));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) XHSI_EFIS_COPILOT_MINS_MODE);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(efis_copilot_mins_mode));
	i++;

    // MFD
	sim_packet.sim_data_points[i].id = custom_htonl((long) XHSI_MFD_MODE);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(mfd_mode));
	i++;

    // AP
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT_AUTOPILOT_AUTOPILOT_STATE);
...
int createEnginesPacket(void) {
	...
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_OVERFLOW_ACF_NUM_TANKS);
	sim_packet.sim_data_points[i].value = custom_htonf((float) tanks);
	i++;
	engines = XPLMGetDatai(num_engines);
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_ENGINE_ACF_NUM_ENGINES);
	sim_packet.sim_data_points[i].value = custom_htonf((float) engines);
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_REVERSER_DEPLOYED);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(reverser_deployed));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_OIL_PRESSURE);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(oil_pressure));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_OIL_TEMPERATURE);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(oil_temperature));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_ANNUNCIATORS_FUEL_PRESSURE);
	sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(fuel_pressure));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_FLIGHTMODEL_WEIGHT_M_FUEL_TOTAL);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(total_fuel));
	i++;

    XPLMGetDatavf(fuel_quantity, fuelfloat, 0, tanks);
	for (e=0; e<tanks; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_COCKPIT2_FUEL_QUANTITY_ + e));
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL_ENGINE_ENGN_N1_ + e));
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL_ENGINE_ENGN_EGT_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL_ENGINE_ENGN_EGT_C_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL2_ENGINES_THRUST_REVERSER_DEPLOY_RATIO_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<tanks; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_AIRCRAFT_OVERFLOW_ACF_TANK_RATIO_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL_ENGINE_ENGN_N2_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL_ENGINE_ENGN_FF_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL_ENGINE_ENGN_OIL_PRESS_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL_ENGINE_ENGN_OIL_TEMP_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_COCKPIT2_ENGINE_INDICATORS_OIL_QUANTITY_RATIO_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(XHSI_FLIGHTMODEL_ENGINE_VIB_ + e));
	...
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_OPERATION_FAILURES_HYDRAULIC_PRESSURE_RATIO1);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(hyd_p_1));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_OPERATION_FAILURES_HYDRAULIC_PRESSURE_RATIO2);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(hyd_p_2));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_HYDRAULICS_INDICATORS_HYDRAULIC_FLUID_RATIO_1);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(hyd_q_1));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_COCKPIT2_HYDRAULICS_INDICATORS_HYDRAULIC_FLUID_RATIO_2);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(hyd_q_2));
	i++;

	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_CONTROLS_ACF_TRQ_MAX_ENG);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(engine_trq_max));
	i++;

    XPLMGetDatavf(engine_trq, engifloat, 0, engines);
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL_ENGINE_ENGN_TRQ_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL_ENGINE_ENGN_ITT_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL_ENGINE_ENGN_ITT_C_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}

	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_CONTROLS_ACF_RSC_REDLINE_PRP);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(prop_rpm_max));
	i++;

    XPLMGetDatavf(prop_rpm, engifloat, 0, engines);
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_COCKPIT2_ENGINE_INDICATORS_PROP_SPEED_RPM_ + e));
        sim_packet.sim_data_points[i].value = custom_htonf( engifloat[e] );
        i++;
	}
	...
	for (e=0; e<engines; e++) {
        sim_packet.sim_data_points[i].id = custom_htonl((long)(SIM_FLIGHTMODEL_ENGINE_ENGN_PROPMODE_ + e));
    ...
    if ( ufmc_ready ) {
        ...
        sim_packet.sim_data_points[i].id = custom_htonl((long) UFMC_N1_1);
        sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(ufmc_n1_1));
        i++;
        sim_packet.sim_data_points[i].id = custom_htonl((long) UFMC_N1_2);
        sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(ufmc_n1_2));
        i++;
        sim_packet.sim_data_points[i].id = custom_htonl((long) UFMC_N1_3);
        sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(ufmc_n1_3));
        i++;
        sim_packet.sim_data_points[i].id = custom_htonl((long) UFMC_N1_4);
    ...
    if ( x737_ready ) {
        ...
        sim_packet.sim_data_points[i].id = custom_htonl((long) X737_N1_PHASE);
        sim_packet.sim_data_points[i].value = custom_htonf((float) XPLMGetDatai(x737_N1_phase));
        i++;
        sim_packet.sim_data_points[i].id = custom_htonl((long) X737_N1_LIMIT_ENG1);
        sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(x737_N1_limit_eng1));
        i++;
        sim_packet.sim_data_points[i].id = custom_htonl((long) X737_N1_LIMIT_ENG2);
...
int createStaticPacket(void) {
	...
	sim_packet.sim_data_points[i].id = custom_htonl((long) XHSI_FLIGHTMODEL_POSITION_NEAREST_ARPT);
	...
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_VIEW_ACF_VSO);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(acf_vso));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_VIEW_ACF_VS);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(acf_vs));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_VIEW_ACF_VFE);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(acf_vfe));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_VIEW_ACF_VNO);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(acf_vno));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_VIEW_ACF_VNE);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(acf_vne));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_VIEW_ACF_MMO);
	sim_packet.sim_data_points[i].value = custom_htonf(XPLMGetDataf(acf_mmo));
	i++;
	sim_packet.sim_data_points[i].id = custom_htonl((long) SIM_AIRCRAFT_OVERFLOW_ACF_VLE);
...
int createFmsPackets(void) {
	...
	long active_entry;
	long total_waypoints;
	long displayed_waypoint = -1;
	long active_waypoint = -1;
...
//XPLMDebugString(msg);
			...
			// if ( lat != 0.0f || lon != 0.0f ) {
				...
				fms_packet[cur_pack].entries[cur_waypoint].type = custom_htonl( (long)type );
				...
				fms_packet[cur_pack].entries[cur_waypoint].altitude = custom_htonl(altitude);
...
//    }
    ...
    for (i = 0; i <= ((total_waypoints-1)/MAX_FMS_ENTRIES_POSSIBLE); i++) {
        ...
        fms_packet[i].nb_of_entries = custom_htonl( total_waypoints );
        fms_packet[i].displayed_entry_index = custom_htonl( displayed_waypoint );
        fms_packet[i].active_entry_index = custom_htonl( active_waypoint );
    }
