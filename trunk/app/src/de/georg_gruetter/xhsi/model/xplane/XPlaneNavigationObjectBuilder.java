...
public class XPlaneNavigationObjectBuilder implements PreferencesObserver {
	...
	public void read_nav_table() throws Exception {
		...
		File file = new File( this.pathname_to_xplane + "/Resources/default data/earth_nav.dat");
	...
	public void read_fix_table() throws Exception {
		...
		File file = new File( this.pathname_to_xplane + "/Resources/default data/earth_fix.dat");
		BufferedReader reader = new BufferedReader( new FileReader( file ));
		String line;
		String[] tokens;
		long line_number = 0;
		...
		while ((line = reader.readLine()) != null) {
			line_number++;
			line = line.trim();
			if ((line_number > 3) && (line.equals("") == false) && (line.equals("99") == false)){
				try {
	...
	public void read_apt_table() throws Exception {
		...
		File file = new File( this.pathname_to_xplane + "/Resources/default scenery/default apt dat/Earth nav data/apt.dat");
		BufferedReader reader = new BufferedReader( new FileReader( file ));
		String line;
		long line_number = 0;
		int info_type;
		...
		while ((line = reader.readLine()) != null) {
			...
			if ((line.trim() != "") && (line.length() > 2)) {
				try {
					...
					} else if ((info_type == 100) && (current_airport_saved == false)) {
						// runway
						float lat = Float.parseFloat(line.substring(36,47));
						float lon = Float.parseFloat(line.substring(48,61));
					//} else if ((info_type == 10) && (current_airport_saved == false)) {
						// runway
						//float lat = Float.parseFloat(line.substring(5,16));
						//float lon = Float.parseFloat(line.substring(17,30));

						nor.add_nav_object(new Airport(airport_name, airport_icao_code, lat, lon));
					  // logger.warning("<" + airport_name + ">, <" + airport_icao_code + ">" + "<" + lat + ">, <" + lon + ">");
